...
generar_informe.py
-------------------
Genera el "Documento de Hallazgos" exigido por el Challenge 02: un PDF de
consultoría dirigido a la junta directiva de TechLogistics S.A.S., con
gráficas extraídas del análisis y una narrativa que responde las 5
preguntas de alta gerencia.

Uso:
    python generar_informe.py

Salida:
    reports/Informe_Hallazgos_TechLogistics.pdf

Se usa matplotlib (no Plotly/Kaleido) para las gráficas estáticas: es más
confiable en un entorno sin navegador y no agrega una dependencia pesada
solo para exportar imágenes.
...
OUTPUT_PDF = os.path.join(REPORTS_DIR, "Informe_Hallazgos_TechLogistics.pdf")

sys.path.insert(0, PROCESSING_DIR)

from integracion import construir_fuente_unica, SLA_ENTREGA_DIAS  # noqa: E402
...
def _figura_a_imagen(fig, ancho_cm=16):
    """Convierte una figura de matplotlib en un flowable Image de reportlab."""
    buf = io.BytesIO()
...
def grafico_q4(df):
    ...
    for _, fila in resumen_cat.iterrows():
        ax.annotate(fila["Categoria"], (fila["Ratio_Reorden"], fila["NPS"]), fontsize=8, xytext=(5, 5), textcoords="offset points")
...
def grafico_q5(df):
    ...
    for _, fila in resumen.iterrows():
        ax.annotate(fila["Bodega_Origen"], (fila["Antiguedad"], fila["Tasa_Soporte"] * 100), fontsize=8, xytext=(5, 5), textcoords="offset points")
...
# ---------------------------------------------------------------------------
# NARRATIVA (mismo cálculo que las 5 preguntas de app.py)
# ---------------------------------------------------------------------------

def calcular_hallazgos(df):
    """Calcula las cifras clave de las 5 preguntas para narrar el informe."""
    hallazgos = {}
    ...
    resumen_cat = (
        df.dropna(subset=["Categoria"])
        .groupby("Categoria")
        .agg(Ratio_Reorden=("Ratio_Stock_Reorden", "mean"), NPS=("Satisfaccion_NPS", "mean"), Margen=("Margen_Utilidad_Pct", "mean"))
        .reset_index()
    )
    mediana_stock = resumen_cat["Ratio_Reorden"].median()
    mediana_nps = resumen_cat["NPS"].median()
    paradoja = resumen_cat[(resumen_cat["Ratio_Reorden"] >= mediana_stock) & (resumen_cat["NPS"] < mediana_nps)]
    ...
    if not resumen_bodega.empty:
        ...
        hallazgos["q5"] = dict(bodega=peor_bodega.name, antiguedad=peor_bodega["Antiguedad"], soporte=peor_bodega["Soporte"])
    else:
...
def _tabla_health_score(nombre_dataset, styles):
    ...
    datos = [
        ...
        [nombre_dataset.capitalize(), f"{fila['antes']:.1f}", f"{fila['despues']:.1f}", f"{fila['delta']:+.1f}"],
    ]
...
def construir_informe():
    ...
    doc.build(story)
    print(f"\nInforme generado: {os.path.normpath(OUTPUT_PDF)}")
    return OUTPUT_PDF
